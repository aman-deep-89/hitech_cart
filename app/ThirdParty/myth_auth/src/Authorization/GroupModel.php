...
class GroupModel extends Model
{
    ...
    public function removeGroupFromAllPermission(int $groupId)
    {
        return $this->db->table('auth_groups_permissions')
            ->where('group_id', $groupId)
            ->delete();
    ...
    public function getData() {
        $res=$this->db->table('auth_groups')->select('*,auth_groups.id as grp_id,auth_groups.name AS grp_name,auth_groups.description AS grp_desc,GROUP_CONCAT(auth_permissions.name) AS per')->join('auth_groups_permissions','auth_groups_permissions.group_id=auth_groups.id','left')->join('auth_permissions','auth_groups_permissions.permission_id=auth_permissions.id','left')->groupBy('auth_groups.id')->get()->getResult();
